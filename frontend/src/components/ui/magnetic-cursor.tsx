...
// `vecteur` exports the class as `Vector2`; the cursor code below refers to it as `Vec2`.
...
interface MagneticCursorProps {
  ...
  /**
   * Change this whenever elements carrying the hover attribute mount or unmount
   * (route/tab changes, modals, lists). The magnetic targets are collected once
   * per effect run, so without a new key late-mounted elements stay inert.
   */
...
export const MagneticCursor: FC<MagneticCursorProps> = ({
    ...
    magneticElements.forEach((el) => {
      ...
      const handlePointerEnter = () => {
        ...
        gsap.to(cursorEl, {
          ...
          duration: 0.3,
          ease: 'power3.out',
          overwrite: true // GSAP 3 spelling of the legacy 'all'
        });
      };
  ...
  }, [disableOnTouch, isTouchDevice, hoverPadding, hoverAttribute, cursorColor, shape, rescanKey]);
